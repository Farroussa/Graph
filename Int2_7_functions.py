...
def menu(matrix):
    ...
    if b == 2:
        ...
        if is_acyclic(matrix) and not negative_arcs(matrix):
            ...
            #p_task_pred = copy.deepcopy(tasks_matrix)
            #transform_pretty_table2(p_task_pred)
            ...
            for i in rank[n]:
                for e in range(len(constraint_matrix)):
                    if i in constraint_matrix[e][2]:
                        liste = [int(x) for x in constraint_matrix[e][2].split(',')]
                        if int(i) in liste:
                            liste.remove(int(i))
                        if len(liste) == 0:
                            constraint_matrix[e][2] = "none"
                        else:
                            constraint_matrix[e][2] = ",".join(map(str, liste))
            n += 1
...
def predecessor(matrix, constraint_table):
    ...
    all_vertices = {item[0] for item in constraint_table}
    predecessors = set()
    for entry in constraint_table:
        if entry[2] != 'none':
            predecessors.update(entry[2].split(','))
    non_predecessor_vertices = all_vertices - predecessors
    non_predecessors_string = ','.join(sorted(non_predecessor_vertices))
    predecessor_matrix.append(non_predecessors_string)
